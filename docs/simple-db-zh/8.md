...
每个节点将对应一个页面。内部节点将通过存储存储子项的页码来指向其子项。 btree 向分页器询问特定页码并返回指向页面缓存的指针。页面按页码顺序依次存储在数据库文件中。

节点需要在页面开头的标题中存储一些元数据。每个节点都将存储它是什么类型的节点，它是否是根节点，以及指向其父节点的指针（以允许查找节点的兄弟节点）。我为每个头字段的大小和偏移量定义常量：
...
除了这些常见的头字段外，叶节点还需要存储它们包含的“单元”数。单元格是键/值对。
...
叶节点的主体是一组单元。每个单元格都是一个键，后跟一个值（序列化行）。
...
基于这些常量，这是叶节点的布局当前的样子：

![Our leaf node format](img/589aa888133cca0e6e724edc7ccf15c7.jpg)

我们的叶节点格式

在标头中使用每个布尔值的整个字节有点空间效率，但这使得编写代码以访问这些值变得更容易。

还要注意最后有一些浪费的空间。我们在标题之后存储尽可能多的单元格，但是剩余的空间不能容纳整个单元格。我们将其留空以避免在节点之间拆分单元格。
...
这些方法返回指向有问题值的指针，因此它们既可以用作 getter，也可以用作 setter。
...
游标表示表中的位置。当我们的表是一个简单的行数组时，我们可以访问只给出行号的行。现在它是一棵树，我们通过节点的页码和该节点内的单元号来识别位置。
...
当我们第一次打开数据库时，数据库文件将为空，因此我们将页面 0 初始化为空叶节点（根节点）：
...
接下来，我们将创建一个用于将键/值对插入叶节点的函数。它将游标作为输入来表示应该插入对的位置。
...
我们还没有实现拆分，所以如果节点已满，我们就会出错。接下来，我们将细胞向右移动一个空间，为新细胞腾出空间。然后我们将新的键/值写入空白区域。
...
有了这些变化，我们的数据库就像以前一样工作！除了现在它更快地返回“表满”错误，因为我们还不能分割根节点。

叶节点可以容纳多少行？

## 打印常量的命令

我正在添加一个新的 meta 命令来打印出一些感兴趣的常量。
...
哦，我们仍然没有按排序顺序存储行。您会注意到`execute_insert()`插入到`table_end()`返回的位置的叶节点中。所以行按插入顺序存储，就像之前一样。
