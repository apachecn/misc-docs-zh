...
我们的数据库允许您插入记录并将其读回，但只要您保持程序运行。如果您终止程序并重新启动它，那么所有记录都将消失。这是我们想要的行为的规范：
...
好的！我们有持久性。这不是最棒的。例如，如果您在未键入`.exit`的情况下终止程序，则会丢失更改。此外，我们将所有页面写回磁盘，甚至是自从我们从磁盘读取它们之后没有更改的页面。这些是我们稍后可以解决的问题。
